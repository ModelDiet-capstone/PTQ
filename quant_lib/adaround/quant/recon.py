# quant_lib/quant/recon.py
import torch
import torch.nn as nn
import logging

from .fake_quant import AdaRoundFakeQuantize, QuantizeBase
...
try:
    from utils.data_utils import DataSaverHook, StopForwardException
except ImportError:
    pass 
...
def reconstruction(model, fp_model, module, fp_module, cali_data, config):
    ...
    # 1. 데이터 캐싱 (Inputs & Outputs)
    # quant_inp: 이전 블록까지 Quantized된 모델을 통과한 출력 (현재 블록의 입력)
    # fp_oup: FP32 모델의 현재 블록 출력 (Target)
