...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, cali_data: torch.Tensor,
                         ...
                         multi_gpu: bool = False, keep_gpu: bool = False):
    """
    ...
    if not act_quant:
        ...
        # Debugging
        print(f"DEBUG: Checking block {block.__class__.__name__}...")
        for name, module in block.named_modules():
            print(f"  - Found module: {name} ({type(module)})")
            # 여기서 QuantModule인지 아닌지 확인
        # ----------------------------------------------------
        
        # 1. 블록 자체(껍데기)에 있는 delta 수집
        if hasattr(block, 'act_quantizer') and block.act_quantizer is not None:
        ...
        # 2. 블록 내부의 자식 모듈들(QuantModule)에 있는 delta 수집
        for name, module in block.named_modules():
            if isinstance(module, QuantModule):
                if hasattr(module, 'act_quantizer') and module.act_quantizer is not None:
                    if hasattr(module.act_quantizer, 'delta') and module.act_quantizer.delta is not None:
                        opt_params.append(module.act_quantizer.delta)
        
        # 파라미터가 zero면 함수 종료 (Skip)
        if len(opt_params) == 0:
            return
    ...
    """else:
        # Use UniformAffineQuantizer to learn delta
        if hasattr(block, 'act_quantizer') and hasattr(block.act_quantizer, 'delta'):
    ...
    cached_inps, cached_outs = save_inp_oup_data(model, block, cali_data, asym, act_quant, batch_size, keep_gpu=keep_gpu)
    if opt_mode != 'mse':
        cached_grads = save_grad_data(model, block, cali_data, act_quant, batch_size=batch_size, keep_gpu=keep_gpu)
    else:
        cached_grads = None
    device = 'cuda'
