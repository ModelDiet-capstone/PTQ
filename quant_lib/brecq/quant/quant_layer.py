...
class QuantModule(nn.Module):
    ...
    def __init__(self, org_module: Union[nn.Conv2d, nn.Linear], weight_quant_params: dict = {},
        ...
        # 1. 가중치 파라미터 등록 (핵심: nn.Parameter 사용)
        self.weight = nn.Parameter(org_module.weight.data.clone())
        
        # 2. 원본 가중치 백업 (Reconstruction Loss 계산용)
        # register_buffer를 써서 GPU 이동은 지원하되, Inference시에는 사용하지 않음
    ...
    def forward(self, input: torch.Tensor):
        # Weight selection based on quantization state
        if self.use_weight_quant:
            weight = self.weight_quantizer(self.weight)
            bias = self.bias
        else:
            # Use original FP32 weights when quantization is OFF
            weight = self.org_weight
            bias = self.org_bias
            
        # Apply convolution/linear operation
        ...
        # Apply activation quantization to OUTPUT (not input)
