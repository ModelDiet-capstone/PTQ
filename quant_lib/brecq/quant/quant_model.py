...
class QuantModel(nn.Module):
    ...
    def quant_module_refactor(self, module: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}):
        ...
        for name, child_module in module.named_children():
            # Check if module type is in specials dict
            if type(child_module) in specials:
                setattr(module, name, specials[type(child_module)](child_module, weight_quant_params, act_quant_params))
            # Also match by class name for cross-library compatibility (e.g., qdrop's BasicBlock)
            elif child_module.__class__.__name__ in ['BasicBlock', 'Bottleneck', 'ResBottleneckBlock', 'InvertedResidual']:
                # Import the Quant wrappers directly
                from quant.quant_block import QuantBasicBlock, QuantBottleneck, QuantResBottleneckBlock, QuantInvertedResidual
                quant_class_map = {
                ...
                QuantClass = quant_class_map.get(child_module.__class__.__name__)
                if QuantClass:
                    setattr(module, name, QuantClass(child_module, weight_quant_params, act_quant_params))
                else:
                    # Fallback: recursively refactor
